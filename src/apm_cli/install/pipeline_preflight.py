...
def _preflight_auth_check(ctx, auth_resolver, verbose: bool) -> None:
    ...
    for dep in ctx.deps_to_install:
        ...
        probe_env = auth_resolver.git_env_for_context(dep_ctx, base_env=dict(_dl.git_env))
        ...
        # for Entra-cached credentials. For generic hosts the same logic
        # applies; widening the carve-out keeps the actual clone path isolated
        # (it builds its own clean env) while giving the preflight probe the
        # best chance to succeed.
        if is_generic:
            for _key in ("GIT_CONFIG_GLOBAL", "GIT_CONFIG_NOSYSTEM", "GIT_ASKPASS"):
                probe_env.pop(_key, None)

        endpoint = dep_ctx.host_info.display_name if dep_ctx else host
        ...
        def _bearer_op(
            ...
            bearer_env = auth_resolver._build_git_env(
                bearer, scheme="bearer", host_kind="ado", base_env=_dl.git_env
            )
