...
| `--clean` | Remove orphaned AGENTS.md files no longer produced by the current primitive set. Nested Git worktrees are excluded from cleanup; run `apm compile --clean` inside that checkout to clean it. For `--target claude`, also removes a stale APM-generated CLAUDE.md once instructions live in `.claude/rules/`. Hand-authored CLAUDE.md files (files without the `<!-- Generated by APM CLI -->` marker) are never deleted. |
