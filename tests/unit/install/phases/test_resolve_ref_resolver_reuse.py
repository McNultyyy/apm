...
def test_semver_resolution_preserves_bearer_and_basic_auth_schemes():
    ...
    # Auth header must be present; non-auth GIT_CONFIG values (e.g.
    # safe.bareRepository=explicit) inherited from the process env are fine.
    ado_auth_headers = {
        value
        for key, value in ado_kwargs["env"].items()
        if key.startswith("GIT_CONFIG_VALUE_") and value.startswith("Authorization:")
    }
    assert ado_auth_headers == {f"Authorization: Bearer {bearer_token}"}
