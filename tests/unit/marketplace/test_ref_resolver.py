...
class TestRefResolverTokenInjection:
    ...
    def test_ado_pat_uses_basic_header_without_url_credentials(self) -> None:
        ...
        # Auth header is appended AFTER any inherited non-auth git config
        # entries (safe.bareRepository, credential.interactive, etc.)
        count = int(env.get("GIT_CONFIG_COUNT", "0"))
        auth_idx = count - 1  # appended last
        assert env[f"GIT_CONFIG_KEY_{auth_idx}"] == "http.extraheader"
        expected = base64.b64encode(b":ado_pat").decode()
        assert env[f"GIT_CONFIG_VALUE_{auth_idx}"] == f"Authorization: Basic {expected}"
